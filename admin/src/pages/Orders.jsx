...
const Orders = () => {
  ...
  const handleApproveApproveOrder = async (orderId, variationId) => {
    try {
      setIsLoading(true);
      const response = await dispatch(
        approveExchange({ orderId, variationId })
      );
      if (response?.payload?.success) {
        setShowOrderDetails(false);
        await handleGetAllOrders(currentPage, itemsPerPage);
        ...
        {showOrderDetails && selectedOrder && (
          <div className="fixed inset-0 z-50 overflow-y-auto">
            <div className="flex items-end  bg-black/40 justify-center  overflow-y-auto pt-4 px-4 pb-20 text-center sm:block sm:p-0">
              ...
              <span
                className="hidden sm:inline-block cursor-pointer sm:align-middle sm:h-screen"
                aria-hidden="true"
              >
                &#8203;
              </span>
                                      ...
                                      <div className="mt-2 flex items-center justify-between">
                                        ...
                                        {!product.exchanged && (
                                          <label className="relative inline-flex items-center cursor-pointer">
                                            <input
                                              type="checkbox"
                                              checked={product.exchanged}
                                              onChange={() =>
                                                handleApproveApproveOrder(
                                                  selectedOrder._id,
                                                  product.variationId
                                                )
                                              }
